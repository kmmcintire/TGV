...
library(dplyr)
library(forcats)
toprow<-ggplot(data=neonatesize, aes(x = GMApara, y=gdaughtersize, fill = Para)) +theme(text = element_text(family = "Times New Roman"))+
  geom_boxplot()+theme_classic()+  
  scale_fill_manual(values =c("#1F1F1F","#898989","")) +
  #scale_pattern_manual(values = c(Exposed = "circle", None = "none")) +
  labs(x = "", y = "Neonate Size", fill = "Exposure") + 
...
###MIDDLE ROW: c1-3 offspring #
###data tgvtryagain, sheet2. MomParasite, Parasite,Clone, sum


middlerow<-ggplot(data=sheet2, aes(x = MomParasite, y=sum, fill = Parasite)) +theme(text = element_text(family = "Times New Roman"))+
  geom_boxplot()+theme_classic()+  
  scale_fill_manual(values =c("#1F1F1F","#898989","")) +
  #scale_pattern_manual(values = c(Exposed = "circle", None = "none")) +
...
###BOttom row: # of clutches
###data tgvtryagain, sheet2. MomParasite, Parasite, clutches

Bottomrow<-ggplot(data=sheet2, aes(x = MomParasite, y=clutches, fill = Parasite)) +theme(text = element_text(family = "Times New Roman"))+
  geom_boxplot()+theme_classic()+  
  scale_fill_manual(values =c("#1F1F1F","#898989","")) +
  #scale_pattern_manual(values = c(Exposed = "circle", None = "none")) +
  labs(x = "Maternal Exposure", y = "Clutches Produced",fill = "Exposure") + 
  facet_grid(.~Clone)+ #separates into facets by clone
  guides(fill = guide_legend(override.aes = list(pattern = "none")))+ theme(legend.position="none")+ theme(text = element_text(size = 17))  +  
